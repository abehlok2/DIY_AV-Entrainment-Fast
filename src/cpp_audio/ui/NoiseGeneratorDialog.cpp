...
public:
  ...
  void resized() override {
    ...
    fileBrowse.setBounds(w - 60, y, 60, h);
    y += h + 8;
    ...
    inputBrowse.setBounds(w - 60, y, 60, h);
    y += h + 8;
    loadButton.setBounds(x, y, 60, h);
    saveButton.setBounds(x + 70, y, 60, h);
    testButton.setBounds(w - 140, y, 60, h);
    generateButton.setBounds(w - 70, y, 60, h);
  }
...
private:
  ...
  TextButton loadButton{"Load"}, saveButton{"Save"}, testButton{"Test"},
  ...
  void buttonClicked(Button *b) override {
    if (b == &fileBrowse) {
      ...
      FileChooser fc("Load Noise Settings", {},
                     String("*") + NOISE_FILE_EXTENSION);
      if (fc.browseForFileToOpen()) {
        NoiseParams p;
      ...
      FileChooser fc("Save Noise Settings", {},
                     String("*") + NOISE_FILE_EXTENSION);
      if (fc.browseForFileToSave(true))
      ...
      auto stream = std::make_unique<MemoryInputStream>(
          mem.getData(), mem.getDataSize(), false);
  ...
  void setParams(const NoiseParams &p) {
    ...
    lfoWaveform.setSelectedId(
        p.lfoWaveform.startsWithIgnoreCase("triangle") ? 2 : 1);
